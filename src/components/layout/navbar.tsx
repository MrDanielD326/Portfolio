    ...
    const handleScroll = () => {
      ...
      const activeSection =
        ["home", "about", "projects", "contact"].find(section => {
          ...
          return (
            element && scrollPosition >= element.offsetTop && scrollPosition < element.offsetTop + element.offsetHeight
          );
        }) || "home";
        ...
        <div className="flex items-center justify-between">
          ...
          {isMobile
            ? <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
              ...
              <SheetContent className="w-[280px] bg-background border-l border-[#888888]/20" side="right">
                  ...
                  {navItems.map((item, index) =>
                    ...
                    >
                      <motion.div
                        ...
                        className={`p-3 rounded-lg transition-all duration-300 ${activeSection === item.href.substring(1)
                      ...
                      <motion.div
                        ...
                        initial={{ scaleY: 0 }}
                        animate={{ scaleY: activeSection === item.href.substring(1) ? 1 : 0 }}
            ...
            : <div className="flex items-center space-x-4 md:space-x-8">
              {navItems.map(item =>
                <a
                  key={item.name}
                  href={item.href}
                  className={`relative flex items-center gap-2 text-sm md:text-base font-medium transition-colors duration-300 ${activeSection === item.href.substring(1) ? "text-[#E0E0E0]" : "text-[#B0B0B0] hover:text-[#E0E0E0]"}`}
                >
                  ...
                  {item.name}
                  {activeSection === item.href.substring(1) &&
                    <motion.div layoutId="activeSection" className="absolute -bottom-1 left-0 right-0 h-0.5 bg-[#888888]" />
                  }
                </a>
              )}
            </div>}
